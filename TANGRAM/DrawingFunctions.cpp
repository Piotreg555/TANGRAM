...
void GUIMyFrame1::DrawWorkspace()
{
	...
	for (Tan* tan : TanTiles) {
		if (tan->isHeld) dc.SetBrush(wxBrush(wxColor(0, 255, 0)));
		else dc.SetBrush((wxBrush(wxColor(200, 200, 200))));
		if (tan->checkMousePosition(MousePositionInWorkspace) && MouseClicked && tan->isHeld) tan->tileOffset = MousePositionInWorkspace;
		if (!tan->visibleInTray) tan->drawInWorkspace(&dc, tan->tileOffset, 1, QPressed, EPressed);
	}
}
...
void GUIMyFrame1::DrawTray()
{
	...
	for (Tan* tan : TanTiles) {
		...
		if (tan->visibleInTray) tan->drawInTray(&dc, wxPoint(0, 0), 1);
	}
}