...
{
if (event.GetKeyCode() == 69) EPressed = true;
if (event.GetKeyCode() == 81) QPressed = true;
}

void GUIMyFrame1::panelWorkspaceOnKeyUp( wxKeyEvent& event )
{
if (event.GetKeyCode() == 69) EPressed = false;
if (event.GetKeyCode() == 81) QPressed = false;
}
...
if (event.LeftIsDown()) MouseClicked = true;
else {
MouseClicked = false;
isAnyTanHeld = false;
}
if(!isAnyTanHeld) pickUp();
